...
static void kv6draw (vx5sprite *spr)
{
	...
	if (cputype&(1<<25))
	{
		...
		/* `movq mm6, qsum0; movq mm7, qsum1` lived here originally —
		 * dead since drawboundcubesse moved to intrinsics that read
		 * qsum0/qsum1 from memory directly. */
...
static void kv6draw_noz(vx5sprite *spr)
{
	...
	if (cputype&(1 << 25))
	{
		...
		/* See kv6draw above — the mm6/mm7 loads are dead now that
		 * drawboundcubesse reads qsum0/qsum1 via memory operands. */
